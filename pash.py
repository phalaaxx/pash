...
class Node(threading.Thread):
    ...
    def shrun(self, cmd):
        if not self._connected:
            return False
        self._commands.append(Command(self.ssh, cmd))
        return True
...
def docfix(method):
    ...
    doclines = list(map(lambda x: x.replace("\t\t", "", 1), method.__doc__.split("\n")))
    callback.__doc__ = "\n".join(doclines[1:-1])
...
class Pash(cmd.Cmd):
    ...
    def do_shell(self, line):
        ...
        for node in filter(lambda n: n.connected(), self.nodes):
            for line in node.last_cmd_output():
                print(
                    template.format(
                        "\033[0;35m{}\033[0m".format(node.config.Name), line
                    )
                )
    ...
    def ListNodes(self):
        ...
        hsize = max(max(map(lambda x: len(x.config.Name), self.nodes)), 8)
        asize = max(max(map(lambda x: len(x.config.Address), self.nodes)), 7)
    ...
    def do_list(self, line):
        ...
        # list groups
        if not len(self.nodes) or (
            len(Items) > 0 and "bundles".startswith(Items[0].lower())
        ):
            self.ListBundles()
            return
    ...
    def default(self, line):
        ...
        if len(PossibleCommands):
            ...
            print(
                "    Maybe you meant one of these: {}".format(
                    ",".join(PossibleCommands)
                )
            )
            return
